...
def create_env():
    ...
    if len(result['computeEnvironments']) == 0:
        ...
        def_json = 'batch/env_template.json'
...
def create_json(name, bucket, dataset, subject, session, credentials=None, log=False):
    """
    Takes parameters to make jsons
    """
    # Set json template filename
    template = "batch/test.json"
    ...
    if subject != None:
        print "path not none"
        cmd[2] = re.sub('(<SUBJECT>)', subject, cmd[2])
    else:
        cmd[2] = re.sub('(<SUBJECT>)', '', cmd[2])
    # set output path
    if session != None:
        cmd[3] = re.sub('(<SESSION>)', session, cmd[3])
    else:
        cmd[3] = re.sub('(<SESSION>)', '', cmd[3])
