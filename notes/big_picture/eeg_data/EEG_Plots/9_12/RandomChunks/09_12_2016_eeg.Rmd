This week I subsetted the EEG data in the time domain and looked across some other paradigms as well.

Some code for EEG Subsetting
```
...
Plots Voltage vs Time for the electrode Cz
```
eegplot <- function(eegdata) {
  library(ggplot2)
  ggplot(eegdata, aes(Time, Cz)) + geom_line() + ggtitle("Voltage of the Cz Electrode Every 30 ms") +
