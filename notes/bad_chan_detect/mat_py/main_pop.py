...
for name in patient_names:
    ...
    outfile = open("/home/nitin/hopkins/neurodata/nicolas/analysis/" + name + "_tot.txt", "w")
    ...
    mat_jp = []
    mat_rej = []
    if not os.path.isfile("/home/nitin/hopkins/neurodata/nicolas/numpy/" + name + "_tot.npz"):
        print "From scratch!"
        # get patient data
        patient, time, zero = list(get_patient(name + ".mat")[i] for i in [0, 1, 4])
        # get rid of 0 electrodes
        test_dat = patient[:, test_dat]
        # reshape data with trials
        num_trials = 4
        test_dat = trials(patient, num_trials)
        ...
        print "MATLAB Done"
        py_jp, py_rej, py_jpG, py_rejG = pop_jointprob(test_dat, list(set(range(111)) - set(zero)), [3], [3])
        print "Python Done"
        mat_jp = np.asarray(mat_jp)
        mat_rej = np.asarray(mat_rej)
        np.savez("/home/nitin/hopkins/neurodata/nicolas/numpy/" + name +"_tot", py_jp=py_jp, py_rej=py_rej, mat_jp=mat_jp, mat_rej=mat_rej)
        print "File saved!"
    else:
        print "Grabbing from saved!"
        load_dat = np.load("/home/nitin/hopkins/neurodata/nicolas/numpy/" + name + "_tot.npz")
